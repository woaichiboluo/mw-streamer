...
#if defined(HAS_EPOLL)
    // epoll相关  [AUTO-TRANSLATED:84d2785e]
    // epoll related
...
#define EventPollerPoolOnStartedArgs EventPollerPool &pool, size_t &size
    ...
    // Returned references may be shared within one owner's execution graph.
    // There is no return to shared scheduling. The pool retains every exclusive
    // instance so their loops are destroyed and joined with the pool.
