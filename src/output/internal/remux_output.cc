...
class RemuxOutput::Impl final {
 ...
 private:
  ...
  void StartPusher() {
    ...
    pusher->setPushCallbackOnce(
        [target = config_.target](const toolkit::SockException& error) {
          if (error) {
            Log::Warning("首次推流失败，ZLM将继续重试：{}，{}", target,
                         error.what());
          } else {
            Log::Info("首次推流成功：{}", target);
          }
        });
  ...
  void Fail(const std::string& reason) {
    ...
    Log::Error("Remux输出目标已失效：{}，{}", config_.target, reason);
    if (!on_failed_) return;
    try {
      on_failed_(reason);
    } catch (const std::exception& error) {
      Log::Error("Remux输出失败通知异常：{}", error.what());
    } catch (...) {
      Log::Error("Remux输出失败通知发生未知异常");
    }
  }
  ...
  void CloseRecording(std::unique_ptr<Target>& target) noexcept {
    ...
    try {
      ...
      Log::Error("关闭录像失败：{}，{}", target->path().string(), error.what());
    } catch (...) {
      Log::Error("关闭录像发生未知异常：{}", config_.target);
    }
    target.reset();
  }
