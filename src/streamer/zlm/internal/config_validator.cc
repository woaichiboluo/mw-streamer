...
void ValidatePlayerConfig(const PlayerConfig& config) {
  ValidateTimeout(config.connect_timeout_ms, "ZLM拉流连接超时时间超出有效范围");
  ValidateTimeout(config.media_timeout_ms, "ZLM媒体超时时间超出有效范围");
}
...
void ValidateRecordingConfig(const RecordingConfig& config) {
  ...
  if (config.hls_segment_duration_ms <= std::chrono::milliseconds::zero() ||
      config.hls_segment_duration_ms > kMaximumDuration) {
...
void ValidateOutputConfig(const OutputConfig& config) {
  constexpr auto kMaximumDuration =
      std::chrono::milliseconds{std::numeric_limits<std::uint32_t>::max()};
  ValidateTimeout(config.pusher.connect_timeout_ms,
                  "ZLM推流连接超时时间超出有效范围");
  if (config.muxer.paced_sender_interval_ms <
          std::chrono::milliseconds::zero() ||
      config.muxer.paced_sender_interval_ms > kMaximumDuration) {
