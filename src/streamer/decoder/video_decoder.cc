...
class VideoDecoder::Impl final {
 public:
  Impl(StreamInfo stream_info, VideoDecoderConfig config)
    ...
    if (config_.backend == VideoDecoderBackend::kCuda) {
      ...
      MW_LOG_INFO("streamer",
                  ...
                  stream_info_.stream_index, codec_->name,
                  context_.get()->width, context_.get()->height,
                  stream_info_.time_base.num, stream_info_.time_base.den);
    }
  }
  ...
  VideoDecodeResult Drain() {
    ...
    drained_ = true;
    MW_LOG_DEBUG("streamer", "视频解码器已排空: stream_index={}",
  ...
  void Flush() {
    ...
    frame_.Unref();
    drained_ = false;
    MW_LOG_DEBUG("streamer", "视频解码器已刷新: stream_index={}",
                 stream_info_.stream_index);
  }
 ...
 private:
  ...
  std::size_t ReceiveFrames(VideoDecodeResult* decode_result = nullptr,
...
VideoDecoder::VideoDecoder(StreamInfo stream_info, VideoDecoderConfig config)
