...
class AudioDecoder::Impl final {
 public:
  ...
  AudioDecodeResult Drain() {
    ...
    drained_ = true;
    MW_LOG_DEBUG("streamer", "音频解码器已排空: stream_index={}",
  ...
  void Flush() {
    ...
    frame_.Unref();
    drained_ = false;
    MW_LOG_DEBUG("streamer", "音频解码器已刷新: stream_index={}",
                 stream_info_.stream_index);
  }
 ...
 private:
  ...
  std::size_t ReceiveFrames(AudioDecodeResult* decode_result = nullptr,
...
AudioDecoder::AudioDecoder(StreamInfo stream_info, AudioDecoderConfig config)
