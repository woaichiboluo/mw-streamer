  ...
  switch (config.type()) {
    case SinkType::kDecoder: {
      ...
      Require(options.video_decoder.backend == VideoDecoderBackend::kSoftware ||
              ...
              config, "未知视频解码后端");
      Require(options.video_decoder.backend != VideoDecoderBackend::kCuda ||
...
std::unique_ptr<Sink> CreateSink(const SinkConfig& config,
  ...
  switch (config.type()) {
    case SinkType::kDecoder:
      return std::make_unique<DecoderSink>(config.id,
                                           Options<DecoderNodeConfig>(config));
    case SinkType::kAnalysisProcessor:
    ...
    case SinkType::kEncoder:
      return std::make_unique<EncoderSink>(config.id,
                                           Options<EncoderNodeConfig>(config));
    case SinkType::kRemux:
...
std::unique_ptr<Pipeline> BuildPipelineWithRuntime(
  ...
  RuntimeLease runtime_lease(runtime);
