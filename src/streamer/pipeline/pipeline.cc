...
class Pipeline::Impl final : public Input::Observer {
 public:
  explicit Impl(std::unique_ptr<Input> input) : input_(std::move(input)) {
    if (!input_) {
  ...
  void Start() {
    ...
    try {
      StartMessages();
      stop_thread_ =
          std::make_unique<Thread>("mw-pipeline-stop", [this]() { RunStop(); });
 ...
 private:
  ...
  void RequestFatalStop(const std::string& error) noexcept {
    ...
    MW_LOG_ERROR("streamer", "Pipeline发生fatal错误: {}", error);
  }
