...
#include "mw/streamer/processor/internal/enum_converter.h"
#include "mw/streamer/processor/internal/execution_context_adapter.h"
 ...
 public:
  ...
  void Stop() noexcept {
    ...
    try {
      ...
      MW_LOG_INFO("processor", "Processor停止完成");
    } catch (const std::exception& error) {
      MW_LOG_ERROR("processor", "Processor停止回调异常，已隔离：{}",
                   error.what());
    } catch (...) {
      MW_LOG_ERROR("processor", "Processor停止回调异常，已隔离：未知异常");
    }
  }
...
const HardwareContext* ProcessorHandler::hardware_context() const noexcept {
