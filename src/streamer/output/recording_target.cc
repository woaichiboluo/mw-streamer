...
class HlsFmp4FileTarget::Recorder final : public mediakit::MP4MuxerMemory {
 public:
  Recorder(const std::filesystem::path& path, const RecordingConfig& config,
           bool preserve_packets)
  ...
  ~Recorder() override {
    try {
      ...
      MW_LOG_ERROR("streamer", "刷新HLS-fMP4录像失败：{}，{}", path_.string(),
                   error.what());
    }
  }
...
Fmp4FileTarget::~Fmp4FileTarget() {
  try {
    ...
    MW_LOG_ERROR("streamer", "关闭fMP4录像失败：{}，{}", path_.string(),
                 error.what());
  }
}
...
HlsFmp4FileTarget::HlsFmp4FileTarget(
    const std::filesystem::path& requested_path,
    const std::vector<mediakit::Track::Ptr>& tracks, RecordingConfig config,
...
HlsFmp4FileTarget::~HlsFmp4FileTarget() {
  try {
    ...
    MW_LOG_ERROR("streamer", "关闭HLS-fMP4录像失败：{}，{}", path_.string(),
                 error.what());
  }
}
