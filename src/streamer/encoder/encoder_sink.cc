 ...
 private:
  ...
  void ConfigureTrack(const StreamInfo& stream) {
    ...
    if (type == AVMEDIA_TYPE_AUDIO) {
      ...
      audio_encoder_ =
          std::make_unique<AudioEncoder>(config_.audio_encoder, audio_index_);
    ...
    video_encoder_ =
        std::make_unique<VideoEncoder>(config_.video_encoder, video_index_);
  ...
  void EncodeTrack(const std::unique_ptr<Encoder>& encoder, const Frame& frame,
                   OperationRecorder& recorder, std::uint64_t input_count) {
  ...
  void CompleteEncoding() {
    ...
    if (audio_encoder_ && CanProcess()) {
      OperationRecorder::Call call(audio_performance_, active_call_);
      audio_encoder_->Drain();
    }
    if (video_encoder_ && CanProcess()) {
      OperationRecorder::Call call(video_performance_, active_call_);
  ...
  void Fail(const char* error, bool fatal = false) noexcept {
    ...
    if (first_failure) {
      MW_LOG_ERROR("streamer", "EncoderSink失败: {}", error);
    }
  }

  EncoderSink& owner_;
  const EncoderSinkConfig config_;
  OperationRecorder audio_performance_{PerformanceType::kAudioEncoder,
                                       PerformanceUnit::kSample,
                                       PerformanceUnit::kPacket};
  OperationRecorder video_performance_{PerformanceType::kVideoEncoder,
...
EncoderSink::EncoderSink(std::string id, EncoderSinkConfig config)
    : Sink(std::move(id), SinkMediaType::kFrame, SinkMediaType::kPacket),
