...
Dictionary MakeOptions(const EncoderProperties& properties) {
  ...
  for (const auto& [key, value] : properties) {
    if (key.empty()) {
      MW_LOG_WARNING("streamer", "忽略键为空的AAC编码器属性");
      continue;
    }
 ...
 public:
  ...
  void Open(const Frame& prototype) {
    ...
    try {
      ...
      ThrowIfError(
          ...
          "配置AAC输入采样格式转换");
      ThrowIfError(swr_init(pending_resampler), "初始化AAC输入采样格式转换");
  ...
  void Drain() {
    ...
    MW_LOG_DEBUG("streamer",
                 "AAC编码器已排空: encoder_name={}, stream_index={}",
 ...
 private:
  ...
  Frame AllocateConvertedFrame(int sample_capacity) const {
    ...
    ThrowIfError(
        ...
        "复制AAC转换输出声道布局");
    ThrowIfError(av_frame_get_buffer(output.get(), 0), "分配AAC转换输出帧");
    return output;
  }
  ...
  void Store(Frame& frame, int sample_count) {
    ...
    ThrowIfError(av_audio_fifo_realloc(fifo_, current_size + sample_count),
                 "扩展AAC FIFO");
  ...
  Frame ReadFrame(int sample_count) {
    ...
    ThrowIfError(av_channel_layout_copy(&frame->ch_layout, &context->ch_layout),
                 "复制AAC编码帧声道布局");
  ...
  void SendFrame(const AVFrame* frame) {
    for (;;) {
      ...
      ThrowIfError(result, frame ? "提交AAC编码帧" : "提交AAC编码结束标记");
      break;
    }
    ReceivePackets();
  }
...
void AudioEncoder::Open(const Frame& prototype) { impl_->Open(prototype); }
