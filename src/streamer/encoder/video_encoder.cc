...
Dictionary MakeOptions(const VideoEncoderConfig& config,
  ...
  for (const auto& [key, value] : config.properties) {
    if (key.empty()) {
      MW_LOG_WARNING("streamer", "忽略键为空的视频编码器属性");
      continue;
    }
    if (key == "bf") {
      int b_frames = 0;
...
class VideoEncoder::Impl final {
 public:
  ...
  void Open(const Frame& prototype) {
    ...
    stream_info_.emplace(StreamInfo::FromCodecContext(*context, stream_index_));
  ...
  void Drain() {
    ...
    MW_LOG_DEBUG("streamer",
                 "视频编码器已排空: encoder_name={}, stream_index={}",
 ...
 private:
  ...
  void SendFrame(const AVFrame* frame) {
    for (;;) {
      ...
      ThrowIfError(result, frame ? "提交视频编码帧" : "提交视频编码结束标记");
      break;
    }
    ReceivePackets();
  }
...
void VideoEncoder::Open(const Frame& prototype) { impl_->Open(prototype); }
