...
class AudioResampler::Impl final {
 public:
  explicit Impl(StreamInfo stream_info) : stream_info_(std::move(stream_info)) {
  ...
  void Drain() {
    ...
    drained_ = true;
    MW_LOG_DEBUG("streamer", "音频重采样器已排空: stream_index={}",
  ...
  void Flush() {
    ...
    drained_ = false;
    MW_LOG_DEBUG("streamer", "音频重采样器已刷新: stream_index={}",
                 stream_info_.stream_index);
  }
 ...
 private:
  ...
  void EnsureInitialized(const AVFrame& input) {
    ...
    const auto* parameters = stream_info_.codec_parameters.get();
    try {
      ThrowIfError(swr_alloc_set_opts2(
                       &pending_context, &parameters->ch_layout,
                       AV_SAMPLE_FMT_FLT, kOutputSampleRate, &input.ch_layout,
  ...
  Frame AllocateOutputFrame(int sample_capacity) const {
    ...
    ThrowIfError(
        ...
        "复制重采样输出声道布局");
    ThrowIfError(av_frame_get_buffer(output.get(), 0), "分配重采样音频帧");
    return output;
  }
...
void AudioResampler::Resample(const Frame& frame) { impl_->Resample(frame); }
